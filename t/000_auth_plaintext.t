...
# just pretend to be enough of Net::AFP to test this UAM
{
    ...
    sub FPLoginExt {
        ...
        return ( ( $p eq $server_password )
            ? $Net::AFP::Result::kFPNoErr
            : $Net::AFP::Result::kFPUserNotAuth );
    }
...
ok(
    ...
    q{AFP modern auth, fails because of too-long password}
);
ok(
    try_to_authenticate_old() == $Net::AFP::Result::kFPUserNotAuth,
    q{AFP old-style auth, fails because of too-long password}
);
...
ok( try_to_change_password($newpass) == $Net::AFP::Result::kFPNoErr,
    q{change password, this can only succeed} );
