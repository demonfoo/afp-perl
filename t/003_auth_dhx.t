...
# just pretend to be enough of Net::AFP to test this UAM
{
    ...
    sub dhx_part1 {
        ...
        my $ciphertext = ${$obj}{ctx}->encrypt(
            pack(
                sprintf( q{a[%d]a[%d]}, @{$obj}{qw(nonce_len signature_len)} ),
                ${$obj}{ $pfx . q{nonce} }
            ),
            @{$obj}{ $pfx . q{K}, q{S2CIV} }
        );
    ...
    sub dhx_part2 {
        ...
        my ( $nonce, @vals ) = unpack(
            sprintf( q{a[%d]%s}, ${$obj}{qw(nonce_len)}, $mask ),
    ...
    sub FPLoginCont {
        ...
        my ( $rc, $pw ) =
          dhx_part2( $obj, sprintf( q{Z[%d]}, ${$obj}{pw_len} ), $ai );
        delete ${$obj}{id};
    ...
    sub FPChangePassword {
        ...
        my ( $rc, $n, $o ) =
          dhx_part2( $obj, sprintf( q{Z[%1$d]Z[%1$d]}, ${$obj}{pw_len} ),
            $the_rest, q{pc_} );
        delete ${$obj}{pc_id};
        if ( $rc != $Net::AFP::Result::kFPNoErr ) {
            return $rc;
